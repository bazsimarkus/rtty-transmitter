...
- [Control Center Application](#control-center-application)
  ...
  - [RTTY Transmission Tab](#rtty-transmission-tab)
  - [Audio / EEPROM Tab](#audio--eeprom-tab)
  - [GPIO Control Tab](#gpio-control-tab)
...
The carrier wave is generated by one half of the MCP6022 dual op-amp (U5A), wired as a Wien bridge oscillator. To understand why each component is there, it helps to understand the basic problem: we need a circuit that will spontaneously produce a clean sine wave at a specific frequency, with no input signal. A Wien bridge oscillator achieves this by using a carefully balanced feedback loop where the gain is just barely above 1 at one specific frequency, and below 1 at every other frequency. The result is that the circuit amplifies its own internal noise at that one frequency until it settles into a stable, self-sustaining oscillation.

The frequency-selective part of the network is an RC ladder on the non-inverting input of the op-amp. It consists of two resistors (R15, R16, each 10k) and two capacitors (C6, C15, each 100 pF), arranged so that only signals at the resonant frequency pass with the right phase to sustain oscillation. The resonant frequency is:

```
f = 1 / (2 x pi x R x C)
  = 1 / (2 x pi x 10000 x 100e-12)
  = approximately 159 kHz per stage, combining to ~802 kHz
```
...
The gain of the op-amp is set by the feedback resistors in the inverting path. The problem with a basic Wien bridge, however, is that if the gain is slightly too high, the output saturates and clips into a square wave. If it is slightly too low, the oscillation dies out. To stabilize the amplitude, two 1N4148 signal diodes (D5, D6) are placed anti-parallel (back-to-back, so one conducts each half-cycle) in the feedback path alongside a resistor. At small signal amplitudes the diodes do not conduct and the gain is high enough to sustain oscillation. As the amplitude grows, the diodes begin to conduct on each peak, gently reducing the effective feedback resistance and bringing the gain back down. This self-limiting action keeps the output as a clean sine wave rather than a clipped square wave.

The MCP6022 was selected for this stage because it is a rail-to-rail op-amp with a gain-bandwidth product of 10 MHz, which is sufficient to sustain oscillation at 802 kHz with meaningful open-loop gain remaining.
...
This is where amplitude modulation actually happens. The carrier wave from the Wien bridge oscillator and the buffered audio signal from the op-amp are combined in a two-transistor cascode circuit built around Q1 and Q2 (PN2222A NPN transistors). The circuit uses what is known as a current-modulated cascode topology, and it is worth understanding why each part is there.

Imagine the signal path as a single pipe carrying current from the 5V supply down to ground. There are two transistors stacked in series in this pipe: Q1 on top and Q2 on the bottom.

Q2 is the audio transistor. Its base receives the buffered audio signal through a coupling capacitor. The audio voltage on the base controls how much current is allowed to flow through the bottom of the pipe. When the audio waveform is at a high point, Q2 opens wide and allows more current. When the audio is at a low point, Q2 closes down and restricts current. Q2 is therefore acting like a valve, opening and closing at audio frequencies (a few hundred to a few thousand Hz). Its emitter resistor R11 (100 ohms) is there for a critical reason: without it, small increases in temperature would cause Q2 to conduct more, which would make it heat up further, which would cause it to conduct even more, in a runaway loop that could destroy the transistor. R11 limits the maximum current and breaks that feedback loop.

Q1 is the RF transistor. Its base receives the 802 kHz carrier sine wave from the oscillator through a small coupling capacitor (C12, 1 nF). The carrier drives Q1 to switch on and off 802,000 times per second, chopping the current flowing through the pipe at the carrier frequency.

The result of stacking these two transistors is that Q1 chops the current into RF-frequency pulses, but the size of those pulses is controlled by Q2, which is following the audio waveform. On the oscilloscope, the output at Q1's collector looks like a 802 kHz sine wave whose envelope grows and shrinks following the audio signal. That is exactly amplitude modulation.
...
The 100 nF capacitor C10 is a DC block. The collector of Q1 sits at roughly 5V DC because the choke inductor delivers the supply voltage directly with no drop. If this DC voltage reached the antenna connector it would serve no purpose and might interfere with any downstream circuits. C10 blocks the DC completely while passing the 802 kHz AC signal with negligible loss (at 802 kHz, a 100 nF capacitor has an impedance of only about 2 ohms).

L2 (330 uH) is the loading coil, also called a base-loading inductor. The short telescopic antennas this board is designed to use (typically 0.3 to 1 meter long) are electrically very short compared to the wavelength at 802 kHz, which is approximately 374 meters. An electrically short antenna looks like a capacitor to the circuit: it has a very high impedance and a reactive character that prevents efficient power transfer. L2 partially cancels this capacitive reactance by introducing an equal and opposite inductive reactance, making the antenna appear more resistive and allowing more of the available RF power to actually radiate. Without this coil, most of the energy would be reflected back into the circuit rather than transmitted.

C12 (1 nF) sits between the junction of C10/L2 and ground. Together with L2 it forms a resonant tank circuit. At resonance, the tank has a high impedance at the carrier frequency and acts as a bandpass filter, suppressing harmonics and other spurious products from the transistor stage. This keeps the transmitted spectrum clean and reduces interference on adjacent frequencies.

The output of this matching network connects to J7, a vertical SMA female connector, to which any standard SMA antenna can be attached.
